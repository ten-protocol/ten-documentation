...
The key feature of [Account Abstraction](https://medium.com/p/2e85bde4c54d) (EIP-4337) is “session keys” (SKs) through a proxy smart contract. SKs allow users to interact with the blockchain without signing every transaction, which is a major UX improvement.
...
The steps below describe the implementation for a game developer—the primary use case for SKs. Note that SKs can be used for any dApp that requires a no‑click UX.
...
### During the game[​](#during-the-game "Direct link to During the game")
...
As a game developer, you are responsible for keeping track of the SK’s balance. You can also query the network for the balance of the SK address. If the SK runs out of balance, you must ask the user to move more funds to the SK.
